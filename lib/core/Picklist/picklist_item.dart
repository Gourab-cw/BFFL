...
class PicklistItem {
  ...
  PicklistItem({required this.id, required this.name, required this.typeId, this.isActive = true, this.order = 0});

  Map<String, dynamic> toMap() {
    return {'id': id, 'name': name, 'typeId': typeId, 'isActive': isActive, 'order': order, 'updatedAt': FieldValue.serverTimestamp()};
  }

  Map<String, dynamic> toJSON() {
    return {'id': id, 'name': name, 'typeId': typeId, 'isActive': isActive, 'order': order};
  }

      ...
      builder: (context) {
        ...
        return StatefulBuilder(
          builder: (context, setState) {
            ...
            final toastWidth = GetPlatform.isMobile ? (screenWidth - 32).clamp(280.0, 420.0) : 400.0;
              ...
              child: Material(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [
                      BoxShadow(color: Colors.black.withAlpha(22), blurRadius: 16, offset: const Offset(0, 6)),
                      BoxShadow(color: getAccentColor().withAlpha(20), blurRadius: 8, offset: const Offset(0, 2)),
                    ],
                  ),
                  child: Row(
                    children: [
                      ...
                      Container(
                        width: 36,
                        height: 36,
                        decoration: BoxDecoration(color: getBackgroundColor(), borderRadius: BorderRadius.circular(10)),
                        child: Icon(getIcon(), size: 20, color: getAccentColor()),
                      ),
                          ...
                          if (withUndoBtn == true) ...[
                            InkWell(
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(color: getBackgroundColor(), borderRadius: BorderRadius.circular(8)),
                                child: TextHelper(text: "Undo", fontsize: 11, fontweight: FontWeight.w700, color: getAccentColor(), padding: EdgeInsets.zero),
                              ),
                            ),
...
String currenyFormater({
  ...
  return '${withCurrency ? '₹' : ''} $extra${modifiedVal.split('').reversed.join('')}$floatPart ${parseDouble(
            data: parseString(data: value, defaultValue: '').replaceAll(',', ''),
            defaultValue: 0.0,
          ) == 0 ? "" : DrCrString}';
}
  ...
  Future<DownloadFileResponse> downloadAndSaveFile({
    ...
    try {
      ...
      if (GetPlatform.isWeb) {
        ...
        final Uint8List dataList = Uint8List.fromList(response.data!);

        await FilePicker.platform.saveFile(
          dialogTitle: 'Choose location to save the file',
          fileName: "${filePathWithName.replaceAll('/', '').replaceAll('PDF', '')}.pdf",
          bytes: dataList,
        );

        file = XFile.fromData(dataList, mimeType: 'application/pdf', name: filePathWithName.split('/').last);
      } else {
        ...
        await dio
            ...
            .whenComplete(() async {
              ...
              if (outputFile == null) {
                throw Exception("Error! file not saved!");
              }
            });
      }

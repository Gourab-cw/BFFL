...
import '../controller/staff_register_controller.dart';
...
class _StaffRegisterState extends State<StaffRegister> {
  final staffRegisterController = Get.find<StaffRegisterController>();
  ...
  void initState() {
    ...
    Future(() async {
      try {
        Loader.startLoading();
        await staffRegisterController.fetchRegister();
      } catch (e) {
  ...
  List<SlotModel> filteredRegister(String searchText) {
    ...
    if (text.isEmpty) {
      return staffRegisterController.register;
    }
    return staffRegisterController.register.where((w) {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<StaffRegisterController>(
      init: staffRegisterController,
      autoRemove: false,
      builder: (staffRegisterController) {
        return AppLoader(
          child: Scaffold(
            appBar: AppBar(
              title: Text('Register'),
              actions: [
                DateRangePicker(
                  ...
                  onValueChange: (v) async {
                    if (staffRegisterController.selectedDate != v) {
                      staffRegisterController.selectedDate = v;
                      try {
                        Loader.startLoading();
                        await staffRegisterController.fetchRegister();
                      } catch (e) {
                      ...
                      staffRegisterController.update();
                    }
                  },
                  selectedDateRange: staffRegisterController.selectedDate,
                ),
              ],
            ),
            body: Column(
              children: [
                    ...
                    if (showSearch)
                      Expanded(
                        child: TextBox(
                          ...
                          autofocus: true,
                          onValueChange: (v) {
                            staffRegisterController.searchTerm = v;
                            staffRegisterController.update();
                          },
                        ),
                      ),
                    ButtonHelperG(
                      ...
                      onTap: () {
                        setState(() {
                          ...
                          if (!showSearch) {
                            staffRegisterController.searchTerm = '';
                            staffRegisterController.update();
                          }
                        });
                      },
                ...
                Expanded(
                  child: Builder(
                    builder: (context) {
                      final list = filteredRegister(staffRegisterController.searchTerm);
